...
import com.InsightMarket.ai.dto.locationchatbot.result.LocationDocumentRowDTO;
...
public class PythonClient {
    ...
    // 트렌드 연관 검색어 생성 요청 -------------------------------------------------------
    public void generateTrends(Long brandId, String brandName, Consumer<PythonTrendResponseDTO> onSuccess, Consumer<Throwable> onError) {
        Map<String, Object> body = new HashMap<>();
        body.put("brandId", brandId);
        body.put("brandName", brandName);

        log.info("[PythonRagClient] 트렌드 수집 요청 전송 -> brandId: {}, brandName: {}", brandId, brandName);

        webClientBuilder
                .baseUrl(pythonBaseUrl)
                .build()
                .post()
                .uri("/api/trends/generate-related")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
                .bodyValue(body)
                .retrieve()
                ...
                .subscribe(
                        ...
                        error -> {
                            log.error("[PythonRagClient] 트렌드 수집 실패: brandId={}, error={}", brandId, error.getMessage());
                            if (onError != null) {
