  ...
  const openPortoneV2 = async (data) => {
    ...
    try {
      ...
      // V2 응답 처리 (오류가 발생하면 response.code가 존재함)
       ...
       //오류란 결제창을 종료시 
     if (response.code !== undefined) { //결제창을 그냥 닫았거나 카드 오류로 돈이 안 나감.
          alert(`결제 취소`);
       try {
      ...
      try {
        ...
        // 서버에서 던진 "결제 금액이 일치하지 않아 자동 환불..."
        const serverErrorMessage = error.response?.data?.message || "결제 검증 중 오류가 발생했습니다.";
