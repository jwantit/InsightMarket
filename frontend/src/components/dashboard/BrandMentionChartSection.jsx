...
const BrandMentionChartSection = ({ brandId, appliedChannels }) => {
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      ...
      setLoading(true);
      try {
        const result = await getBrandMentionChart(brandId, appliedChannels, unit);
  ...
  return (
    ...
    <div className="w-full h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-[15px] font-extrabold text-gray-800 tracking-tight text-sm">언급량 추이</h3>
            ...
            <Tooltip 
              ...
              formatter={(value, name, props) => {
                if (name === "count") {
                  ...
                  return [`네이버: ${naver}, 유튜브: ${youtube}`, "전체 언급량"];
                }
                return [value, name];
              }}
            />
